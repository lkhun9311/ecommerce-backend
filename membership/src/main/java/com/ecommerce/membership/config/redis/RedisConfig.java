...
public class RedisConfig {
    ...
    public CacheManager cacheManager(RedisConnectionFactory redisConnectionFactory, ObjectMapper redisObjectMapper) {
        ...
        // UserMe 캐시 TTL 설정
        customCacheConfig.put(RedisCacheKey.USER_ME, defaultCacheConfig
                .entryTtl(Duration.ofSeconds(RedisCacheTtl.USER_ME_EXPIRE_SEC))); // 사용자 정의 캐시 TTL 설정

        // JwtToken 캐시 TTL 설정
        customCacheConfig.put(RedisCacheKey.JWT_TOKEN, defaultCacheConfig
                .entryTtl(Duration.ofSeconds(RedisCacheTtl.JWT_TOKEN_EXPIRE_SEC))); // 사용자 정의 캐시 TTL 설정
