...
public class RedisConfig {
    ...
    public CacheManager cacheManager(RedisConnectionFactory redisConnectionFactory, ObjectMapper redisObjectMapper) {
        ...
        // UserOrderCurrent 캐시 TTL 설정
        customCacheConfig.put(RedisCacheKey.USER_ORDER_CURRENT, defaultCacheConfig
                .entryTtl(Duration.ofSeconds(RedisCacheTtl.USER_ORDER_CURRENT_EXPIRE_SEC))); // 사용자 정의 캐시 TTL 설정

        // UserOrderHistory 캐시 TTL 설정
        customCacheConfig.put(RedisCacheKey.USER_ORDER_HISTORY, defaultCacheConfig
                .entryTtl(Duration.ofSeconds(RedisCacheTtl.USER_ORDER_HISTORY_EXPIRE_SEC))); // 사용자 정의 캐시 TTL 설정

        // UserOrderRead 캐시 TTL 설정
        customCacheConfig.put(RedisCacheKey.USER_ORDER_READ, defaultCacheConfig
                .entryTtl(Duration.ofSeconds(RedisCacheTtl.USER_ORDER_READ_EXPIRE_SEC))); // 사용자 정의 캐시 TTL 설정
