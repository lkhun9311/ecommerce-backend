...
public class RedisConfig {
    ...
    public CacheManager cacheManager(RedisConnectionFactory redisConnectionFactory, ObjectMapper redisObjectMapper) {
        ...
        // StoreProductRegister 캐시 TTL 설정
        customCacheConfig.put(RedisCacheKey.STORE_PRODUCT_REGISTER, defaultCacheConfig
                .entryTtl(Duration.ofSeconds(RedisCacheTtl.STORE_PRODUCT_REGISTER_EXPIRE_SEC))); // 사용자 정의 캐시 TTL 설정

        // StoreProductSearchStoreId 캐시 TTL 설정
        customCacheConfig.put(RedisCacheKey.STORE_PRODUCT_SEARCH_STORE_ID, defaultCacheConfig
                .entryTtl(Duration.ofSeconds(RedisCacheTtl.STORE_PRODUCT_SEARCH_STORE_ID_EXPIRE_SEC))); // 사용자 정의 캐시 TTL 설정
